...
-- Add subscription tier (only "pro" supported)
ALTER TABLE users ADD COLUMN IF NOT EXISTS subscription_tier VARCHAR(20)
    CHECK (subscription_tier IN ('pro', NULL));
...
COMMENT ON COLUMN users.subscription_tier IS 'User subscription tier: pro (or null for no subscription)';
